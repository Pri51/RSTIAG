...
class Attention_path_len(nn.Module):
    def __init__(self, n_hidden, sen_len,path_num):
        super(Attention_path_len, self).__init__()
    ...
    def forward(self, x):
        ...
        alpha = F.softmax(alpha.reshape(-1, self.path_num,1, self.sen_len), dim = -1)
        # alpha is of shape (batch_size * doc_len, 1, sen_len)
        x = torch.matmul(alpha, x).reshape(-1, self.path_num,self.n_hidden*2)
...
def get_batch_data_pair(x, sen_len, doc_len, y_position, y_cause, y_pair, distance,adj, doc_id_list,paths,rels, adj_path,batch_size, test=False):
    for index in batch_index(len(y_cause), batch_size, test):
        ...
        y_pair[index], adj[index], distance[index], [doc_id_list[i] for i in index],paths[index],rels[index],adj_path[index]]
